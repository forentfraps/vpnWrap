...
// to the 16 raw bytes used inside the VLESS request. We don't import
// google/uuid because the parsing is 20 lines and adding a dep is silly.
func parseUUID(s string) ([16]byte, error) {
	...
	if len(b) != 16 {
		return u, errors.New("uuid hex did not decode to 16 bytes")
	}
	copy(u[:], b)
	return u, nil
}
